 ...
 * (`arfima` returned a closure, `holtWintersForecast` was a free
 * function, `createLSTM` / `createAttentionModel` returned object
 * literals) with a single polymorphic `Forecaster` base class.
 *
...
export class Forecaster {
   ...
   *
   * @param {Array<number>} _history Time-ordered `H` estimates.
   * @return {number} Predicted `H`.
   * @abstract
   */
  predict(_history) {
    throw new Error(`${this.constructor.name}.predict() is not implemented`);
  }
 ...
 *
 * @private
 * @param {number} rows Rows rows.
 * @param {number} cols Cols cols.
...
export class LstmForecaster extends Forecaster {
  constructor(opts = {}) {
    ...
    this._sigmoid = (x) => 1 / (1 + Math.exp(-x));
    this._tanh = (x) => Math.tanh(x);
  }
  ...
  _step(x, hPrev, cPrev) {
    ...
    for (let j = 0; j < this.hiddenSize; j++) {
      ...
      f[j] = this._sigmoid(sumF);
      iGate[j] = this._sigmoid(sumI);
      cTilde[j] = this._tanh(sumC);
      o[j] = this._sigmoid(sumO);
    }
    const c = new Array(this.hiddenSize);
    const h = new Array(this.hiddenSize);
    for (let j = 0; j < this.hiddenSize; j++) {
      c[j] = f[j] * cPrev[j] + iGate[j] * cTilde[j];
      h[j] = o[j] * this._tanh(c[j]);
    }
    return {h, c};
  }
...
export class AttentionForecaster extends Forecaster {
  ...
  predict(series) {
    ...
    const queries = projected.map((p) => this._matVecMul(this.Wq, p));
    const keys = projected.map((p) => this._matVecMul(this.Wk, p));
    const values = projected.map((p) => this._matVecMul(this.Wv, p));
    ...
    const result = this._matVecMul(this.WoAttn, output);
    for (let i = 0; i < this.hiddenSize; i++) {
      result[i] /= n;
    }
    return result[0];
...
/**
 * Computes the (truncated) fractional difference of a series.
 *
 * @private
 * @param {Array<number>} data Input series.
 * @param {number} d Differencing parameter.
 * @param {number=} lag Maximum lag (default `50`).
