...
import {runKalmanFilter} from '../inference/filtering.js';
...
export class KsSignificanceTest extends HypothesisTest {
  ...
  run(data, opts = {}) {
    ...
    if (!Number.isFinite(D) || D < 0) {
      throw new Error('KsSignificanceTest: D must be a non-negative finite number');
    }
...
export class BootstrapConfidenceInterval extends HypothesisTest {
  ...
  run(data, opts = {}) {
    ...
    if (typeof estimator !== 'function') {
      throw new Error('BootstrapConfidenceInterval: estimator must be a function');
    }
...
/* Silence ESLint: kept for parity with consumers of the strategy base. */
