 ...
 * - {@link RoughFsvModel}
 * - {@link FractionalOuModel} (covers both `fOU` and `exactOU`)
 * - {@link MultifractionalPreModel} (covers both `mPRE` and `mPREExact`)
 *
 * Every concrete class is also registered under its short name in
 * `lib/strategies/registry.js` via `registerStrategies()`.
 */

import {nextGaussian, generateFractionalNoise, generateCorrelatedGaussian} from '../stochastic-generators.js';
...
export class StochasticModel {
  ...
  simulate(opts = {}) {
    throw new Error(
      `${this.constructor.name}.simulate() is not implemented`,
    );
  }
  ...
  price(sim, opts = {}) {
    throw new Error(
      `${this.constructor.name}.price() is not implemented`,
    );
  }
}
...
export class RoughBergomiModel extends StochasticModel {
  ...
  simulate(opts = {}) {
    ...
    for (let p = 0; p < nPaths; p++) {
      ...
      for (let t = 1; t <= nSteps; t++) {
        const vt = xi * Math.exp(eta * I[t] - driftFactor * Math.pow(t * dt, 2 * H));
  ...
  price(sim, opts = {}) {
    ...
    for (let p = 0; p < sim.paths.length; p++) {
      ...
      for (let t = 0; t < nSteps; t++) {
        ...
        pricePath[t + 1] = pricePath[t] * Math.exp(mu * dt + vol * dW[t] * Math.sqrt(dt));
      }
...
export class RoughFsvModel extends StochasticModel {
  ...
  simulate(opts = {}) {
    ...
    for (let t = 1; t <= nSteps; t++) {
      ...
      const volDiff = nu * Math.pow(prev, alpha) * Math.sqrt(dt) * nextGaussian();
      let roughComp = 0;
      for (let j = 0; j < t; j++) {
        roughComp += kernel[t - 1 - j] * fGn[j];
      }
      roughComp *= Math.sqrt(dt);
      volPath[t] = Math.max(1e-6, prev + volDrift + volDiff + roughComp * nu * 0.1);
    }
 ...
 *
 * `opts.discretization` selects between `euler-maruyama` (default) and
 * the `exact` Riemann–Liouville integral discretization.
 */
export class FractionalOuModel extends StochasticModel {
  ...
  simulate(opts = {}) {
    ...
    const discretization = opts.discretization || 'euler-maruyama';
    const times = new Array(nSteps + 1);
    times[0] = 0;
    for (let t = 1; t <= nSteps; t++) times[t] = t * dt;
    const path = new Float64Array(nSteps + 1);
 ...
 *
 * `opts.discretization` selects between `local-holder` (default) and
 * the `exact` time-varying Riemann–Liouville integral.
 */
export class MultifractionalPreModel extends StochasticModel {
  ...
  simulate(opts = {}) {
    ...
    const x0 = opts.x0 !== undefined ? opts.x0 : 0.0;
    const discretization = opts.discretization || 'local-holder';
...
/**
 * Registers every concrete stochastic model in the supplied registry
 * under its short identifier.
 *
 * @param {import('./registry.js').Registry<StochasticModel>} registry
 *   Strategy registry to populate.
 * @return {void}
 */
export function registerStochasticModels(registry) {
  registry.register('rBergomi', () => new RoughBergomiModel());
  registry.register('rFSV', () => new RoughFsvModel());
  registry.register('fOU', () => new FractionalOuModel());
  registry.register('mPRE', () => new MultifractionalPreModel());
}