...
export class StochasticModel {
   ...
   *
   * @param {Object} _opts Model-specific options.
   * @return {SimulationResult} Simulated paths + time grid.
   * @abstract
   */
  simulate(_opts = {}) {
    throw new Error(`${this.constructor.name}.simulate() is not implemented`);
  }
   ...
   *
   * @param {SimulationResult} _sim Output of {@link simulate}.
   * @param {Object} _opts Price-SDE options.
   * @return {PriceResult} Simulated prices.
   */
  price(_sim, _opts = {}) {
    throw new Error(`${this.constructor.name}.price() is not implemented`);
  }
}
