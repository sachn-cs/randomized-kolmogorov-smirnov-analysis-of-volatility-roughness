...
export class Hurstify {
  ...
  constructor(config = {}) {
    ...
    this.#sampler = config.sampler instanceof Sampler
      ? config.sampler
      : defaultSampler(this.blockSize);
    this.#objective = config.objective instanceof KsObjective
      ? config.objective
      : chooseKsObjective(this.scales, this.weights);
    this.#optimizer = resolveOptimizer(config.optimizer || this.optimizerType);
  }
  ...
  validateConfig() {
    if (!Number.isFinite(this.hMin) || !Number.isFinite(this.hMax)) {
      throw new HurstifyError('hMin and hMax must be finite', 'E_INVALID_BOUNDS');
    }
    if (this.hMin >= this.hMax) {
      throw new HurstifyError('hMin must be strictly less than hMax', 'E_INVALID_BOUNDS');
    }
    if (this.hMin <= 0 || this.hMax >= 1) {
      throw new HurstifyError('hMin/hMax must lie in the open interval (0, 1)', 'E_INVALID_BOUNDS');
    }
    if (this.sampleSize <= 0) {
      throw new HurstifyError('sampleSize must be positive', 'E_INVALID_SAMPLE_SIZE');
    }
    if (this.iterations <= 0) {
      throw new HurstifyError('iterations must be positive', 'E_INVALID_ITERATIONS');
    }
    if (this.blockSize !== null && this.blockSize <= 0) {
      throw new HurstifyError('blockSize must be positive when provided', 'E_INVALID_BLOCK_SIZE');
    }
    if (this.scales && this.weights && this.scales.length !== this.weights.length) {
      throw new HurstifyError('weights length must equal scales length', 'E_WEIGHTS_MISMATCH');
    }
  }
  ...
  #estimateSingleRaw(window, opts = {}) {
    if (!window || window.length === 0) {
      throw new HurstifyError('window must be a non-empty array', 'E_EMPTY_WINDOW');
    }
    ...
    if (scales && scales.length > 0) {
      ...
      sortedSamples = scales.map((scale) => {
        ...
        return sampler.draw(inc, sampleSize).slice().sort((a, b) => a - b);
      });
    } else {
      ...
      sortedSamples = [
        sampler.draw(inc1, sampleSize).slice().sort((a, b) => a - b),
        sampler.draw(inc2, sampleSize).slice().sort((a, b) => a - b),
      ];
    }
  ...
  estimate(window, opts = {}) {
    ...
    if (k <= 0) throw new HurstifyError('iterations must be > 0', 'E_INVALID_ITERATIONS');
    for (let i = 0; i < k; i++) {
      try {
    ...
    if (valid === 0) {
      throw new HurstifyError('All optimization iterations failed', 'E_ALL_ITERATIONS_FAILED');
    }
    return sum / valid;
  }
  ...
  rolling(series, windowSize, step = 1, onProgress) {
    ...
    if (step <= 0) throw new HurstifyError('step must be > 0', 'E_INVALID_STEP');
    const results = [];
