/**
 * @fileoverview H-forecasting primitives for hurstify.
 *
 * Provides lightweight forecasting tools that consume a stream of
 * historical `H` estimates (typically the output of `Hurstify.rolling`)
...
export function createLSTM(opts = {}) {
  ...
  const Wf = xavierInit(hiddenSize, inputSize + hiddenSize);
  const Wi = xavierInit(hiddenSize, inputSize + hiddenSize);
  const Wc = xavierInit(hiddenSize, inputSize + hiddenSize);
  const Wo = xavierInit(hiddenSize, inputSize + hiddenSize);
...
export function createAttentionModel(opts = {}) {
  ...
  const Wq = xavierInit(hiddenSize, hiddenSize);
  const Wk = xavierInit(hiddenSize, hiddenSize);
  const Wv = xavierInit(hiddenSize, hiddenSize);
  const WoAttn = xavierInit(hiddenSize, hiddenSize);
