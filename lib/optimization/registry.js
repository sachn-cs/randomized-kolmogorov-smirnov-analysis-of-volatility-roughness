/**
 * @fileoverview Optimizer registry and "safe" wrapper for Hurstify.
 *
 * Each algorithm in `optimization/` is tailored for general use. Here we
 * adapt them to the **uniform signature** the Hurstify estimator expects:
 *
 ...
 * return value. The upper-level `Hurstify.estimate` then either skips or
 ...
 * propagating. The intent is to keep `Hurstify.estimate` resilient: a bad
 * iteration should be logged and skipped, not abort the whole batch.
 *
 * @param {function} opt Optimizer function with the standard Hurstify
 ...
 *   unknown (in which case `Hurstify` falls back to `'brent'`).
 */
 ...
 * return a function with the standard Hurstify signature
 * `(f, hMin, hMax, [h0]) => number`.
 *
 * @param {string} name Optimizer identifier (used in
 *   `Hurstify({optimizerType: name})`).
